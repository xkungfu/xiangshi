        ...
        if self.top != "All":
            if self.random == True:
                ...
                for _ in range(self.top):
                    key, value = random.choice(result)
                    output[key] = value

                    key, value = random.choice(result2)
                    output2[key] = value
            else:
                #按顺序选前top个值
                sorted = list(self.SortDict(result))[:self.top]
                sorted2 = list(self.SortDict(result))[:self.top]

                output = dict((y, x) for x, y in sorted)
                output2 = dict((y, x) for x, y in sorted2)
...
res = sim.cossim("data2/test1.txt", "data2/test3.txt")